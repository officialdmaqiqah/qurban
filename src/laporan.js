    ...
    const renderBagiHasil = async (netProfit, currentConfig) => {
        ...
        if(inpOwner && inpTeam) {
            ...
            inpOwner.onchange = async () => {
                ...
                await supabase.from('master_data').upsert({ id: 'md-porsi-bagi-hasil', key: 'PORSI_BAGI_HASIL', val: newCfg }, { onConflict: 'key' });
                init();
            };
        }
    ...
    const renderAnalytics = (goats, trxs, fin, start, end) => {
        ...
        const isOpex = f => {
            ...
            const isEx = kl.includes('bayar supplier') || kl.includes('pelunasan supplier') || kl.includes('komisi') || 
                         ...
                         isRef || kl.includes('penarikan') || kl.includes('modal') || kl.includes('prive') || kl.includes('investasi') || kl.includes('pinjaman') ||
