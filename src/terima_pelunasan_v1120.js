...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderList = async () => {
        ...
        // POPULATE DATALIST (FIXED v11.20)
        if (listOrders) {
    ...
    selOrder.addEventListener('input', async () => {
        ...
        if (!val) { boxInfoOrder.style.display = 'none'; formBayar.style.display = 'none'; return; }
    ...
    const openRefundModal = (trx) => {
        ...
        document.getElementById('inpTglRefund').value = window.getLocalDate();
            ...
            try {
                ...
                for (const trx of trxs) {
                    ...
                    let total = history.reduce((sum, h) => {
                        ...
                        if (h.tipe === 'pemasukan') {
                            // Abaikan Titipan Agen, Komisi, Karkas, SULAM, TUMBAL
