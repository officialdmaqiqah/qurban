            ...
            try {
                ...
                for (const trx of trxs) {
                    ...
                    let total = history.reduce((sum, h) => {
                        ...
                        if (h.tipe === 'pemasukan') {
                            // KRITIKAL: Hanya abaikan yang BENAR-BENAR Titipan Dana Agen atau Komisi/Karkas
                            // Jangan asal block kata "titipan" karena ada "Titipan Dana Kambing" yang merupakan pembayaran sah
                            if (cat === 'titipan dana agen' || cat.includes('komisi') || cat.includes('karkas') || cat === 'penjualan karkas') return sum;
                            return sum + h.nominal;
                        } else {
                            // SEMUA Pengeluaran (Refund, Pengembalian, Tarik) harus mengurangi saldo
                            return sum + h.nominal; 
                        }
                    }, 0);
