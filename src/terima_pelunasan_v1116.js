...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderList = async () => {
        ...
        // POPULATE DROPDOWN
        selOrder.innerHTML = '<option value="">-- Cari ID / Nama Konsumen --</option>';
        filtered.sort((a,b) => b.id.localeCompare(a.id)).forEach(t => {
            const opt = document.createElement('option');
            opt.value = t.id;
            opt.textContent = `${t.id} - ${t.customer?.nama || '-'}`;
    ...
    window.showAuditDetail = async (trxId) => {
        ...
        if (trx.history_bayar && trx.history_bayar.length > 0) {
            trx.history_bayar.forEach(h => {
                msg += `<div style="border-bottom:1px solid #eee; padding:5px 0;">`;
            ...
            try {
                ...
                for (const trx of trxs) {
                    ...
                    const history = (allFins || []).filter(f => {
                        ...
                        let reason = "";
                        if (f.id === 'PAY-1778819459130') {
                            console.log(`[DEBUG YANA] Meneliti Pembayaran Yana terhadap Order ${tId}...`);
                            console.log(`RelatedID di DB: ${fId}, Target Order: ${tId}`);
                        }

                        if (fId === tId) reason = "ID MATCH (PASTI)";
                        else if (fId && fId !== tId) {
                            if (f.id === 'PAY-1778819459130') console.log(`[DEBUG YANA] Pembayaran ini DITOLAK karena sudah terkunci ke: ${fId}`);
                            return false;
                        }
                    ...
                    let total = history.reduce((sum, h) => {
                        ...
                        if (h.tipe === 'pemasukan') {
                            if (cat === 'titipan dana agen' || cat.includes('komisi') || cat.includes('karkas') || cat === 'penjualan karkas') return sum;
                            return sum + h.nominal;
                        } else {
                            // HANYA Pengeluaran yang sifatnya mengembalikan uang konsumen (Refund/Pengembalian) yang boleh mengurangi saldo
                            // Abaikan Komisi Agen, Kas Operasional, atau Biaya lainnya
                    ...
                    // Jika tidak ada history sama sekali, jangan reset ke 0 jika sebelumnya sudah ada data manual (safety)
