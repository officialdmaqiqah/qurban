...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const refreshKambingDropdown = async (filterText = '') => {
        ...
        available.slice(0, 50).forEach(k => { // Limit to 50 for performance
            ...
            const sexEmoji = (k.sex || '').toLowerCase().startsWith('b') ? '♀️' : '♂️';
            opt.value = signature;
            opt.textContent = `No.${k.no_tali} [${k.lokasi || '-'}] - ${sexEmoji} ${k.warna_tali}${extraStat} - ${formatNum(k.harga_kandang)}`;
            ...
            div.innerHTML = `
                <div class="cart-item-header">
                    <div class="cart-item-id">
                        ...
                        <span style="font-size:1.1rem; line-height:1;">${(item.sex || '').toLowerCase().startsWith('b') ? '♀️' : '♂️'}</span>
            ...
            trxs.forEach(t => {
                ...
                (t.items || []).forEach((it, idx) => {
                    ...
                    exportData.push({
                        ...
                        'WA 1': t.customer?.wa1 || '',
                        'WA 2': t.customer?.wa2 || '',
                        'Alamat / Jalan': addr.jalan || '-',
                        'Desa / Dusun': addr.desa || '-',
                        'Kecamatan': addr.kec || '-',
                        'Kabupaten': addr.kab || '-',
