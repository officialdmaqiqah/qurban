...
async function check() {
    const { data: trx } = await supabase.from('transaksi').select('*').eq('id', 'TRX00031').single();
    const { data: fins } = await supabase.from('keuangan').select('*').eq('related_trx_id', 'TRX00031');
